...
describe 'generated runs' do
  (1..ROUNDS).each do |round|
    describe "ROUND #{round}" do
      describe 'meta_datum_licenses_for_random_resource_type' do
        include_context :meta_datum_for_random_resource_type
        let(:meta_datum_licenses) { meta_datum('licenses') }
              ...
              context 'if the response is 200' do
                ...
                it 'it holds the proper uuid array value' do
                  if response.status == 200
                    value.map { |v| v['id'] }.each do |license_id|
                      expect(MetaDatum::License.find_by(meta_datum_id: resource.data['id'],
