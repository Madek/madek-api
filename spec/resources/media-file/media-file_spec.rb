...
describe "MediaFile Resource" do
  ...
  describe "requesting a non existing media-resource" do
    let :resource do
      authenticated_json_roa_client.get.relation("media-file").get("id" => SecureRandom.uuid)
    end
    let :response do
      resource.response
    end
  ...
  describe "the image media file" do
    ...
    describe "the media-file resource" do
      let :resource do
        authenticated_json_roa_client.get.relation("media-file").get("id" => media_file.id)
      end
