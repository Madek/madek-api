    ...
    include_context :json_roa_client_for_authenticated_admin_user do
      describe "removing a user from the group via DELETE" do
        it "responds with 204" do
          expect(
            client.get.relation("group").get(id: @group.id).relation("user").delete(user_id: @user.id).response.status
          ).to be == 204
        end

        it "effectively removes the user from the group" do
          client.get.relation("group").get(id: @group.id).relation("user").delete(user_id: @user.id)
