    ...
    include_context :json_roa_client_for_authenticated_admin_user do
      describe "adding the user to the group via put" do
        it "responds with 200" do
          expect(
            client.get.relation("group").get(id: @group.id).relation("user").put(user_id: @user.id).response.status
          ).to be == 200
        end

        it "effectively adds the user to the group" do
          client.get.relation("group").get(id: @group.id).relation("user").put(user_id: @user.id)
