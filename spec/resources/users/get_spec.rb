    ...
    include_context :json_roa_client_for_authenticated_admin_user do
      context "retriving a standard user" do
        ...
        it "has the proper data, sans :searchable and :previous_id" do
          expect(get_user_result.data.with_indifferent_access.slice(:id, :email, :login, :person_id)).to be == @user.attributes.with_indifferent_access.slice(:id, :email, :login, :person_id)
        end
      end
