...
describe "filtering media entries" do
  ...
  context "by me_ permissons" do
    ...
    context "me_get_metadata_and_previews for a user" do
      include_context :json_roa_client_for_authenticated_user do
        context "two media_entries, both have public permissions for the current, user " \
                "one with and one without public permission" do
          before :each do
            other_users = users.reject { |u| u == user }

            @me_private = FactoryBot.create(:media_entry,
                                            responsible_user: other_users.first,
                                            is_published: true,
                                            get_metadata_and_previews: false,
                                            get_full_size: false)

            @me_public = FactoryBot.create(:media_entry,
                                           responsible_user: other_users.first,
                                           is_published: true,
                                           get_metadata_and_previews: true,
                                           get_full_size: true)

            [@me_private, @me_public].each do |me|
          ...
          it "only me_get_metadata_and_previews gets two entries" do
            mes = get_media_entries("me_get_metadata_and_previews" => "true")

            expect(mes.count).to be == 2
          end

          it "setting the filter to false gets one entry" do

            mes = get_media_entries("me_get_metadata_and_previews" => "true",
                                    "public_get_metadata_and_previews" => "false")

            expect(mes.count).to be == 1
