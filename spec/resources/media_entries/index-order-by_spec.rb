      ...
      context "response of ordering by metadatum string (title usually)" do
        ...
        specify "media_entries are ordered by metadatum string" do
          media_entry_ids_from_response = response
            ...
            .map { |me| me[:id] }
          media_entry_ids_ordered_by_titles = titles.filter { |t| media_entry_ids_from_response.include? t[:media_entry_id] }.sort_by { |t| t[:string] }.map { |t| t[:media_entry_id] }

          expect(media_entry_ids_ordered_by_titles.to_a.length).to eq media_entry_ids_from_response.to_a.length

          expect(media_entry_ids_ordered_by_titles.to_a.sort).to be == media_entry_ids_from_response.to_a.sort
        end
      end
    end
    ...
    context "ordering by order param" do
      ...
      context "manual" do
        ...
        it "returns shuffled entries every time" do
          expect(update_arcs_with_positions.map(&:media_entry_id)).not_to eq(update_arcs_with_positions.map(&:media_entry_id))
        end
